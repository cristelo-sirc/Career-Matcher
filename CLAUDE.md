...
- **Web unit tests:** 130 passing (components, hooks, utilities, URL encoding round-trips)
...
- **Deployment status:** Git repo initialized, initial commit created, GitHub Pages workflow ready — awaiting push to `cristelo-sirc/Career-Matcher` on GitHub (see Deployment section below)
- **Reviews completed:**
...
npm run build         # Next.js production build (static export → out/)
npm run start         # serve the static export locally (npx serve out)
npm run lint          # ESLint + tsc --noEmit
npm run test          # Vitest — 130 component + hook + utility tests
npm run test:watch    # Vitest watch mode
npm run test:e2e      # Playwright E2E tests — 191 tests across 5 browsers (requires `npm run build` first)
...
web/                              ← Web application (all phases complete, deployment-ready)
  src/
    app/                          — Next.js App Router pages
      layout.tsx                  — Root layout: fonts, metadata, CSP meta tag, viewport config
      page.tsx                    — Landing page
      quiz/
        page.tsx                  — Prompt flow experience
      results/
        page.tsx                  — Results dashboard + export (share URL, copy text, print, start over)
      about/
    ...
    components/
      ...
      prompts/
        PromptCard.tsx            — Single prompt display (with arrow-key navigation)
        ...
        index.ts                  — Barrel export
      results/
        ProfileSummary.tsx        — Dimension preference visualization (all 8 dims)
        MatchCard.tsx             — Single job match (expandable with reasons, friction, metadata)
        FitBadge.tsx              — Color-coded fit band badge (color + icon + text)
      ...
      layout/
        ...
        index.ts                  — Barrel export
    hooks/
      useQuizState.ts             — Quiz state management (reducer + persistence)
    lib/
      storage.ts                  — StorageAdapter: sessionStorage → Map fallback
      url-state.ts                — Encode/decode resolved profile for shareable URLs
      constants.ts                — Section labels, prompts per section, transitions
    styles/
      globals.css                 — Tailwind v4 @theme tokens, class-based dark mode, CSS custom properties, @media print, safe-area insets
    __tests__/
      components/                 — 24 component test files (Vitest + Testing Library)
      hooks/                      — Hook unit tests
      core-import.test.ts         — Verifies core library imports work
      storage.test.ts             — StorageAdapter tests
      url-state.test.ts           — URL encoding/decoding round-trip tests
      setup.ts                    — Test setup (jsdom, Testing Library matchers)
  e2e/                            — Playwright E2E tests (W-10)
    happy-path.spec.ts            — Landing → complete 32-prompt quiz → results page with matches
    navigation.spec.ts            — Back button, page refresh mid-quiz, direct URL to results, 404
    keyboard.spec.ts              — Full quiz using Tab/Enter/Arrow keys, skip link, focus order
    accessibility.spec.ts         — axe-core WCAG 2.1 AA scan on every page (light + dark mode)
    export.spec.ts                — Share URL round-trip, copy-as-text, print trigger, Start Over
    mobile.spec.ts                — iPhone SE (375px), 320px minimum viewport, touch target sizes
    dark-mode.spec.ts             — Toggle persistence across pages, system preference, user override
    incognito.spec.ts             — Quiz and dark mode with sessionStorage disabled
  scripts/
    generate-icons.js             — Node.js script (sharp) to generate PWA icons + OG image
  public/
  ...
  playwright.config.ts            — 5 browser configs (Desktop Chrome/Firefox/Safari, Mobile Chrome/Safari)
  next.config.ts                  — Static export config, @core alias, extensionAlias
  eslint.config.mjs               — ESLint v9 flat config with jsx-a11y (ignores scripts/)
  lighthouserc.json                — Lighthouse CI config (>= 95 targets for all categories)
  postcss.config.mjs              — PostCSS for Tailwind
  tsconfig.json                   — Extends root, adds JSX + @core path alias
  vitest.config.ts                — Vitest config (excluded from Next.js tsconfig)
  package.json                    — Web-specific dependencies
  QA_CHECKLIST.md                 — Manual iPhone/iPad testing checklist (W-10)

.github/
  workflows/
    ci.yml                        — Lint + test + build for core and web; E2E tests; Lighthouse audit
```
...
| Animation | **CSS animations** | Evaluated in W-1: Framer Motion (~30-40KB gzipped) would exceed the 200KB budget. Using CSS `@keyframes` + `transition` instead (zero JS cost). Respects `prefers-reduced-motion`. |
...
**Tailwind Design Tokens (defined via `@theme` directive in `web/src/styles/globals.css`):**
```
Colors:
...
**Import Strategy:**
The web app imports from the core library via a `@core` path alias (e.g.,
`import { PROMPTS, processResponses } from '@core/index.js'`). The alias is
configured in `web/next.config.ts` (webpack `resolve.alias` + `extensionAlias`
to resolve `.js` imports to `.ts` files) and `web/tsconfig.json` (paths). This
avoids a separate build/publish step during development.
...
```
          ╱ ╲
         ╱ E2E╲            191 tests — Playwright, 5 browser configs
        ╱───────╲           Full user flows, accessibility, dark mode, export, mobile
       ╱ Integr. ╲         ~25 tests — Vitest + Testing Library
      ╱─────────────╲       Component composition, hooks, state management
     ╱    Unit       ╲      ~105 tests — Vitest
...
**Total: 399 tests** (78 core + 130 web unit + 191 E2E)
...
npm run test        # All 130 component/hook/utility tests pass
npm run build       # Next.js static export builds cleanly
npm run test:e2e    # All 191 Playwright E2E tests pass (requires build first)
```
...
See `web/QA_CHECKLIST.md` for the manual device testing guide.
...
**Web application:** All phases (W-1 through W-11) complete. The app is deployment-ready.

**Highest priority:** Push to GitHub and deploy via GitHub Pages, then user acceptance
testing with actual teens (DESIGN_REVIEW.md finding VAL-05).

### Implementation Summary

| Phase | Status | Key Deliverable |
|-------|--------|----------------|
| W-1 | ✓ Complete | Next.js 15 + Tailwind v4 + @core alias + ESLint + CI |
| W-2 | ✓ Complete | 8 UI primitives (Button, Card, Badge, ProgressBar, DimensionBar, Tooltip, SkipLink, ThemeToggle) |
| W-3 | ✓ Complete | Layout (Header, Footer), Landing page, About page, 404 |
| W-4 | ✓ Complete | 32-prompt quiz flow, option cards, progress, back nav, state persistence |
| W-5 | ✓ Complete | Results dashboard, profile summary, match cards, replay, empty state |
| W-6 | ✓ Complete | Print CSS, shareable URL encoding, copy-as-text, start over |
| W-7 | ✓ Complete | Accessibility audit: focus trap, arrow keys, CSP hardening |
| W-8 | ✓ Complete | Privacy/COPPA verified: zero PII, zero cookies, zero third-party scripts |
| W-9 | ✓ Complete | Bundle verified under budget, PWA manifest in place |
| W-10 | ✓ Complete | 191 Playwright E2E tests (5 browsers), WCAG contrast fixes, dark mode infrastructure fix, platform CSS, manual iPhone/iPad QA passed |
| W-11 | ✓ Complete | PWA icons (192/512/180/32), OG image, favicon, robots.txt, CI/CD pipeline (4 jobs), Lighthouse CI config |
...
- **Repository:** `cristelo-sirc/Career-Matcher` on GitHub
- **Live URL (once deployed):** `https://cristelo-sirc.github.io/Career-Matcher/`

### What's Already Done (2026-02-09)

1. **Git repository initialized** in the `Consolidated` folder with a clean `main` branch.
   Single initial commit (`3f9f002`) containing the complete app — 123 files, 25,132 lines.
   This is a fresh start — the old branches documented in `BRANCH_AUDIT.md` have been
   consolidated into this single commit.

2. **`.gitignore` created** — excludes `node_modules/`, build output (`dist/`, `web/out/`,
   `web/.next/`), test artifacts (`playwright-report/`, `playwright-results/`), OS files,
   and editor files.

3. **GitHub Actions workflows created:**
   - `.github/workflows/ci.yml` — Runs on every push/PR to `main`. Two jobs:
     - **Core Library:** `npm ci` → lint → test (78 tests) → build
     - **Web App:** `npm ci` → lint → test (130 tests) → build
   - `.github/workflows/deploy.yml` — Runs on push to `main`. Builds the web app and
     deploys the static export (`web/out/`) to GitHub Pages via `actions/deploy-pages@v4`.

4. **Next.js configured for GitHub Pages:**
   - `web/next.config.ts` updated with `basePath` and `assetPrefix` set to `/Career-Matcher`
     in production. This ensures all links and assets work correctly at the
     `github.io/Career-Matcher/` subpath.
   - Development (`npm run dev`) still works at `localhost:3000/` with no base path.

5. **Pre-deployment verification passed (2026-02-09):**
   - 78 core library tests: all passing
   - 130 web unit tests: all passing
   - ESLint + TypeScript: zero errors
   - Next.js production build: succeeds, all pages under 200KB budget

6. **Minor fixes applied during deployment prep:**
   - `web/e2e/export.spec.ts` — replaced `any` casts with proper `unknown` types (ESLint)
   - `web/e2e/accessibility.spec.ts` — fixed `AxeBuilder.Result` type reference (TypeScript)
   - `web/vitest.config.ts` — added `exclude: ["e2e/**", "node_modules/**"]` so Vitest
     doesn't try to run Playwright E2E files (they use a different test runner)

7. **GitHub CLI (`gh` v2.86.0) installed** at `~/.local/bin/gh` (no Homebrew needed).
   Not yet authenticated — GitHub's device authentication servers were returning HTTP 503
   at the time of setup (2026-02-09).

### What's Pending — Step-by-Step Instructions

These steps can be done by anyone with access to the Mac and the GitHub account.
No coding knowledge required — just copy-paste the commands.
...
- Follow the prompts (copies a one-time code, opens browser to authorize)

If the browser method still fails (GitHub 503 errors), use a Personal Access Token instead:
1. Go to https://github.com/settings/tokens
2. Click "Generate new token (classic)"
3. Check these scopes: **repo**, **workflow**, **read:org** (under admin:org)
4. Copy the token
5. Run: `echo "YOUR_TOKEN_HERE" | ~/.local/bin/gh auth login --with-token`

Verify it worked:
```bash
~/.local/bin/gh auth status
```
You should see "Logged in to github.com as cristelo-sirc".

**Step 2: Push the code to GitHub**

The existing GitHub repo has old branches from earlier development. Since we're starting
fresh, we need to force-push our new clean `main` branch:

```bash
cd "/Users/cristelogarza/Claude Code/Career Matcher All Files/Consolidated"
git remote add origin https://github.com/cristelo-sirc/Career-Matcher.git
git push --force -u origin main
```

Note: If you get an error "remote origin already exists", run this first:
```bash
git remote set-url origin https://github.com/cristelo-sirc/Career-Matcher.git
```
Then retry the `git push --force -u origin main` command.

**Why `--force`:** The old repo has branches (`claude/review-dev-plan-ubpu4`, etc.) but
no `main` branch with matching history. Force-push replaces the remote with our clean
starting point. This is safe because all old work has been consolidated into this commit
(see `BRANCH_AUDIT.md` for the full consolidation audit).

**Step 3: Enable GitHub Pages**

1. Go to https://github.com/cristelo-sirc/Career-Matcher/settings/pages
2. Under **"Source"**, select **GitHub Actions** (not "Deploy from a branch")
3. Click **Save**

That's it. The `deploy.yml` workflow will trigger automatically on the push and build
the site. Within 2–3 minutes, the app will be live at:

**https://cristelo-sirc.github.io/Career-Matcher/**

**Step 4: Verify the deployment**

Check the deployment status at:
https://github.com/cristelo-sirc/Career-Matcher/actions

Look for the "Deploy to GitHub Pages" workflow — it should show a green checkmark.
Then visit the live URL and verify:
- Landing page loads
- Quiz flow works (answer all 32 prompts)
- Results page shows matches
- Dark mode toggle works
- Share URL copies correctly
- Print layout looks clean (Ctrl/Cmd+P)

### Post-Deployment Tasks

After the site is live:

1. **Set `metadataBase`** in `web/src/app/layout.tsx` to the production URL so Open Graph
   images resolve correctly for social media sharing:
   ```typescript
   metadataBase: new URL('https://cristelo-sirc.github.io/Career-Matcher/'),
   ```
   Then commit and push:
   ```bash
   cd "/Users/cristelogarza/Claude Code/Career Matcher All Files/Consolidated"
   git add web/src/app/layout.tsx
   git commit -m "Set metadataBase for social sharing images"
   git push
   ```

2. **Clean up old remote branches** (optional) — the old `claude/*` branches on GitHub
   are no longer needed. Delete them from the GitHub branches page:
   https://github.com/cristelo-sirc/Career-Matcher/branches
   Click the trash can icon next to each:
   - `claude/review-project-design-TBQDW` (duplicate)
   - `claude/career-matcher-foundation-HWrR7` (subset)
   - `claude/review-dev-plan-W9AVA` (merged)
   - `claude/review-dev-plan-ubpu4` (merged)
   - `claude/audit-branch-organization-K2YjW` (merged)

3. **User acceptance testing** with actual teens (DESIGN_REVIEW.md finding VAL-05).
   Every review has identified this as the critical missing piece. The app is
   feature-complete and should be tested with real users as soon as possible.

4. **Revoke the GitHub Personal Access Token** if one was created during deployment.
   Go to https://github.com/settings/tokens and delete the `career-matcher-deploy`
   token. The GitHub CLI will continue to work with its cached credentials.
...
- The URL must include the repo name: `.../Career-Matcher/` (with trailing slash)

**Links/styles broken on the deployed site:**
- This means the `basePath` config isn't working. Verify `web/next.config.ts` has
  `basePath: isProd ? "/Career-Matcher" : ""` and `assetPrefix: isProd ? "/Career-Matcher/" : ""`
- Rebuild and push: `cd web && npm run build` then commit and push

**Quiz works locally but not on GitHub Pages:**
- The app is 100% client-side — no server needed. If it works on `localhost:3000`, it
  should work on GitHub Pages. Check browser console (F12) for JavaScript errors.

**Dark mode not working:**
- Fixed in W-10. If it still doesn't work, verify `globals.css` contains
  `@custom-variant dark (&:where(.dark, .dark *));`

**"remote origin already exists" error:**
- Run `git remote set-url origin https://github.com/cristelo-sirc/Career-Matcher.git`
  then retry the push.
...
## Phase W-10 Completion Log

Phase W-10 (Cross-Device QA) completed successfully. Summary of work:

### E2E Test Infrastructure
- Installed Playwright with 5 browser configs: Desktop Chrome, Firefox, Safari (WebKit), Mobile Chrome (Pixel 7), Mobile Safari (iPhone 14)
- Installed `@axe-core/playwright` for automated WCAG 2.1 AA accessibility scanning
- Created `web/playwright.config.ts` and 8 E2E test files in `web/e2e/`
- Added `test:e2e` and `test:e2e:ui` scripts to `web/package.json`
- **Result:** 191 tests passing, 0 failing, 9 skipped (clipboard tests skipped on non-Chromium browsers)

### Critical Bugs Found and Fixed

| # | Issue | Root Cause | Fix |
|---|-------|-----------|-----|
| 1 | **Dark mode completely broken** | Tailwind v4 defaults to `@media (prefers-color-scheme: dark)` for `dark:` variants, but the app uses class-based `.dark` toggle on `<html>` | Added `@custom-variant dark (&:where(.dark, .dark *))` to `globals.css` |
| 2 | Button text contrast (3.55:1) | `bg-primary-600` (#0d9488) with white text fails WCAG AA 4.5:1 | Changed to `bg-primary-700` (#0f766e) — now 4.97:1 |
| 3 | Header nav link contrast (4.35:1) | `text-text-muted` on semi-transparent `bg-surface/95` header | Changed inactive links and theme toggle to `text-text` |
| 4 | DimensionBar selected pill contrast (2.48:1) | `bg-primary-500` with white text | Changed to `bg-primary-700` — now 5.47:1 |
| 5 | DimensionBar unselected pill contrast (4.29:1) | `text-text-muted` on `bg-primary-100` | Changed to `text-primary-800` (light) / `text-primary-300` (dark) |
| 6 | Fit band badge colors failing contrast | Strong (3.58:1), Stretch (3.07:1), Unlikely (4.39:1) | Darkened CSS theme variables: Strong→#047857, Stretch→#b45309, Unlikely→#4b5563 |
| 7 | Quiz page missing h1 heading | axe-core `page-has-heading-one` rule | Added `<h1 className="sr-only">Career-Matcher Quiz</h1>` |
| 8 | axe-core false positives from fade-in animation | `<main>` has 200ms opacity animation; axe-core scanned mid-animation | Added `waitForFadeIn()` helper (350ms delay) in accessibility tests |
| 9 | axe-core false positives from dark mode transition | `transition-colors` causes intermediate computed colors | Added 500ms wait after dark mode toggle before scanning |
| 10 | WebKit skip link test failure | WebKit doesn't update URL hash on anchor clicks | Browser-specific assertion: check `href` attribute on WebKit |

### Platform-Specific CSS Added
- `viewport-fit: cover` via Next.js `Viewport` export (iPhone notch/Dynamic Island)
- `env(safe-area-inset-*)` padding on body (left, right, bottom)
- `overscroll-behavior-y: none` on body (prevents iOS rubber-band bounce in quiz)

### Manual Device Testing
- Created `web/QA_CHECKLIST.md` with step-by-step testing guide
- iPhone testing: all checklist items passed
- iPad testing: all checklist items passed

### Files Modified in W-10
- `web/src/styles/globals.css` — class-based dark mode directive, fit band color fixes, platform CSS
- `web/src/components/ui/Button.tsx` — primary variant `bg-primary-600` → `bg-primary-700`
- `web/src/components/ui/SkipLink.tsx` — focus background `bg-primary-600` → `bg-primary-700`
- `web/src/components/ui/DimensionBar.tsx` — pill contrast fixes (selected + unselected)
- `web/src/components/layout/Header.tsx` — inactive nav link color to full text color
- `web/src/components/ui/ThemeToggle.tsx` — icon color to full text color
- `web/src/app/quiz/page.tsx` — added visually-hidden h1 heading
- `web/src/app/layout.tsx` — added `Viewport` export with `viewportFit: "cover"`
- `web/src/__tests__/components/Button.test.tsx` — updated test assertions for new class
- `web/package.json` — added Playwright + axe-core deps, new scripts

### Files Created in W-10
